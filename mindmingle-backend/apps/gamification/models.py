...
class UserLevel(models.Model):
    ...
    def _check_badges(self):
        """Auto-award badges based on reputation + give bonus points"""

        from django.db import transaction

        with transaction.atomic():

            # Get all eligible badges
            eligible_badges = Badge.objects.filter(
                points_required__lte=self.total_reputation
            )
            ...
            for badge in eligible_badges:
                if badge.id not in existing_badges:
                    ...
                    # Give bonus reputation when any badge earned
                    Reputation.objects.create(
                        user=self.user,
                        points=20,
                    ...
                    # Update total reputation safely using F expression
                    self.total_reputation = F('total_reputation') + 20
                    self.save(update_fields=['total_reputation'])
                    self.refresh_from_db()
